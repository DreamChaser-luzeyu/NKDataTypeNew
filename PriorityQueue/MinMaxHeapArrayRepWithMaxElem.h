...
class MyMinMaxHeap {
public:
    MyMinMaxHeap(const vector<T>& src, const T& maxElem, const T& minElem);
    ...
    void MinHeapOptimized(int sortedNum);
...
private:
    ...
    int getMaxChildIndex(int parentIndex) const;
...
void MyMinMaxHeap<T>::push(const T& elem) {          // 复杂度：O(log n), O(height)
