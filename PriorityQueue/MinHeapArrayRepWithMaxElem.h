#ifndef _MINHEAPARRAYREP_H
#define _MINHEAPARRAYREP_H
#include <vector>
#include <iostream>
...
class MyMinHeap {
public:
    MyMinHeap(const vector<T>& src, const T& minElem, const T& maxElem);
...
private:
    ...
    bool isMinHeap(int beginIndex) const;
    bool isMinHeap(int beginIndex, const T& temp) const;
    int getMinChildIndex(int parentIndex) const;           // returns the reference of the larger child
    vector<T> data;
    int validSize;
    T minElem;
    T maxElem;
...
void MyMinHeap<T>::push(const T& elem) {          // 复杂度：O(log n), O(height)
    data[validSize + 1] = elem;
    data.push_back(maxElem);
    data.push_back(maxElem);
    validSize ++;
    ...
    while(temp < data.at(parentIndex)) {            // 比父节点大就循环
        ...
        // if(indexNow == 0) {break;}               // 因为有了minElem作为根节点，所以无需判断
    }
    data[indexNow] = temp;
...
void MyMinHeap<T>::pop() {                      // O(log n)
    ...
    for(indexNow = 1; !isMinHeap(indexNow, temp);) {
        int minChildIndex = getMinChildIndex(indexNow);
        data[indexNow] = data[minChildIndex];
        indexNow = minChildIndex;
    }
...
template <class T>
T MyMinHeap<T>::popRemove() {                         // O(log n)
    int lastElemIndex = this->validSize;
    // std::cout << "lastElemIndex : " << lastElemIndex;
    int indexNow;
    T temp = data[lastElemIndex];               // 保存最后一个节点
    T returnVal = data[1];                      // 保存要返回的元素  
    data[lastElemIndex] = this->maxElem;        // 把最后一个节点抹掉
    ...
    if(validSize == 0) {data[1] = this->maxElem;}
    return returnVal;
}


template <class T>
MyMinHeap<T>::MyMinHeap(const vector<T>& src, const T& minElem, const T& maxElem) {
...
template <class T>
void MyMinHeap<T>::buildHeapOptimized(int sortedNum) {    // 从后往前遍历，从上往下走
    ...
    for(int indexNow = beginIndex; indexNow >= 1; indexNow --) { // 当前下标为indexNow
        ...
        while(!isMinHeap(dstIndex, temp)) {               // 判断以temp为根节点的【元子树】是否满足最大堆
            int srcIndex = getMinChildIndex(dstIndex);    // 获取dstIndex对应的最大子节点下标
...
template <class T>
void MyMinHeap<T>::outputArray(ostream& out) {// 这是一个输出函数，可以输出数组的内容
    out << "Array : ";
...
// template <class T>
// void MyMinHeap<T>::erase(const T& elem) {

// }

template <class T>
int MyMinHeap<T>::getParentIndex(int nodeIndex) const {// 获取父节点下标
    // if(nodeIndex < 0) {
...
template <class T>
int MyMinHeap<T>::getChildIndex(int nodeIndex) const {// 获取左子节点下标
...
template <class T>
int MyMinHeap<T>::getFirstLeafNodeIndex() const {// 获取从前往后第一个叶子节点的下标
    int size = validSize;
    return size / 2 + 1;
}

template <class T>
int MyMinHeap<T>::getFirstLeafNodeIndex(int n) const {// 获取从前往后第一个叶子节点的下标
                                                      // 已经删去最后一个元素
...
template <class T>
bool MyMinHeap<T>::isMinHeap(int beginIndex) const {// 判断beginIndex的【元子树】是否满足最大堆
    if(beginIndex >= getFirstLeafNodeIndex()) {return true;}
    T minChild = data.at(getMinChildIndex(beginIndex));
    T elemNow = data.at(beginIndex);
    if(elemNow <= minChild) {return true;}
    return false;
}

template <class T>
bool MyMinHeap<T>::isMinHeap(int beginIndex, const T& temp) const {// 判断beginIndex的【元子树】是否满足最大堆
    // std::cout << "First leaf index : " << getFirstLeafNodeIndex();
    if(beginIndex >= getFirstLeafNodeIndex()) {return true;}
    T minChild = data.at(getMinChildIndex(beginIndex));
    if(temp <= minChild) {return true;}
    return false;
}

template <class T>
int MyMinHeap<T>::getMinChildIndex(int parentIndex) const {// 获取beginIndex的最大子节点下标
    ...
    int rightChildIndex = leftChildIndex+1 <= maxIndex ? 
                          leftChildIndex+1 : -1;
    if(leftChildIndex == -1) {
        throw "getMinChildIndex() : No child : leaf node";
    } // impossible
